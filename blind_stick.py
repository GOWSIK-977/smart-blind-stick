...
class SmartBlindStick:
    def __init__(self, mongodb_uri=None):
        ...
        self.cap = None
        for i in range(5):
            # Try with DirectShow first to prevent Windows MSMF stride/padding bugs
        ...
        print("\n" + "="*60)
        print("✅ SYSTEM READY!")
        print(f"   Arduino: {'✅ Connected' if self.arduino.connected else '⚠️ Not Connected (Software Mode)'}")
        print(f"   Camera: {'✅ OK' if self.cap else '⚠️ Test Pattern'}")
        print(f"   TTS: {'✅ OK' if self.tts_available else '⚠️ Disabled'}")
        print(f"   Database: {'✅ OK' if self.db.enabled else '⚠️ Disabled'}")
        print("="*60 + "\n")
    ...
    def generate_frames(self):
        ...
        while True:
            ...
            # Arduino status
            if self.arduino.connected:
                cv2.putText(frame, "Arduino: ✅ Connected (Buzzer + Vibrator)", (10, y_offset + 25), 
            ...
            # Ensure array is contiguous to prevent stride distortion (horizontal repeating)
                ...
                try:
                    ...
                    if data.get('type') == 'register':
                        ...
                        if lat is not None and lng is not None:
                            ...
                            if self.db.enabled:
                                self.db.log_system_event('LOCATION_UPDATE', f"Location: {self.current_location['address']}")
...
<body>
    <div class="container">
        ...
        <div class="card">
            ...
            <div id="map" style="height: 300px; border-radius: 12px; margin-top: 10px; margin-bottom: 10px; border: 1px solid rgba(255,255,255,0.1);"></div>
            <button onclick="openGoogleMaps()" style="width:100%; padding:10px; background:#4caf50; border:none; border-radius:8px; color:white; cursor:pointer;">🗺️ Open in Google Maps</button>
        </div>
    </div>
    ...
    <script>
            ...
            if (navigator.geolocation) {
                navigator.geolocation.watchPosition(
                    (position) => {
                        ...
                        // Try to geocode to get a pretty address name
                        ...
                        if (geocoder) {
                            geocoder.geocode({ location: latlng }, (results, status) => {
                                ...
                                sendLocationUpdate(lat, lng, address);
                            });
                        } else {
                            sendLocationUpdate(lat, lng, `${lat.toFixed(6)}, ${lng.toFixed(6)}`);
                        }
                    },
                    (error) => {
        ...
        function connectWebSocket() {
            ...
            ws.onopen = () => {
                ...
                // Start mobile GPS tracking once WS is active
                startTracking();
            };
        ...
        function updateUI(data) {
            ...
            if (data.location) {
                ...
                const locText = `📍 ${data.location.address || 'Unknown location'}<br>📌 ${lat.toFixed(6)}, ${lng.toFixed(6)}`;
                document.getElementById('locationInfo').innerHTML = locText;
                
                // Keep center and marker updated in real time as user moves
                if (map && marker) {
